...
public class QS implements CommandExecutor {
	...
	private void setPrice(CommandSender sender, String[] args) {
		if (sender instanceof Player && sender.hasPermission("quickshop.changeprice")) {
			Player p = (Player) sender;
			if (args.length < 2) {
	...
	private void cleanall(CommandSender sender) {
		if (sender.hasPermission("quickshop.cleanall")) {
			sender.sendMessage(MsgUtil.getMessage("command.cleaning"));
			Iterator<Shop> shIt = plugin.getShopManager().getShopIterator();
			int i = 0;
			while (shIt.hasNext()) {
				...
				try {
					if (shop.getLocation().getWorld() != null && (shop.isSelling() || shop.isBuying()) && shop.getRemainingStock() == 0 && shop instanceof ContainerShop) {
						ContainerShop cs = (ContainerShop) shop;
						if (cs.isDoubleShop())
							continue;
						shIt.remove(); // Is selling and buying, but has no stock, and is a chest shop, but is not a double shop. Can be deleted safely.
						i++;
					}
				} catch (IllegalStateException e) {
					shIt.remove(); // The shop is not there anymore, remove it
			...
			MsgUtil.clean();
			sender.sendMessage(MsgUtil.getMessage("coshopOwner mmand.cleaned", "" + i));
			return;
		}
		sender.sendMessage(MsgUtil.getMessage("no-permission"));
		return;
	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		if (args.length > 0) {
			...
			if (subArg.equals("unlimited")) {
				...
				clean(sender);
				return true;
			} else if (subArg.equals("cleanall")) {
				cleanall(sender);
				return true;
	...
	public void sendHelp(CommandSender s) {
		...
		if (s.hasPermission("quickshop.changeprice"))
